...
def create_engine_and_tables(db_url: str | None = None):
    ...
    with engine.connect() as conn:
        ...
        try:
            conn.execute(text("ALTER TABLE saved_image ADD COLUMN source TEXT NOT NULL DEFAULT 'article'"))
        except Exception:
            pass  # column already exists
        conn.execute(text("CREATE INDEX IF NOT EXISTS ix_saved_image_source ON saved_image(source)"))
